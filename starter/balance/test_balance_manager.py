...
class TestBalanceManager(unittest.TestCase):
    ...
    def test_manager_state(self):
        ...
        cmd1 = Command.from_transaction(Transaction(100, TransactionCategory.INCOME), self.balance)
        c.add(cmd1)
        self.assertEqual(c._pointer, 0)
        cmd2 = Command.from_transaction(Transaction(100, TransactionCategory.INCOME), self.balance)
        c.add(cmd2)
        self.assertEqual(c._pointer, 1)
    ...
    def test_manager_undo(self):
        ...
        cmd3 = Command.from_transaction(Transaction(100, TransactionCategory.INCOME), self.balance)
        c.add(cmd3)
        self.assertEqual(c._pointer, 1)
        self.assertEqual(len(c._history), 2) # it gets overwritten, so the history length should be 2 not 3
    ...
    def test_overwrite_redo_history(self):
        ...
        self.balance.apply_transaction(Transaction(30, TransactionCategory.EXPENSE))  # New transaction should have cleared the redo history, so 50 has gone
        with self.assertRaises(Exception):
    ...
    def test_order(self):
        self.balance.apply_transaction(Transaction(100, TransactionCategory.INCOME))
        self.balance.apply_transaction(Transaction(80, TransactionCategory.INCOME))
        self.balance.apply_transaction(Transaction(60, TransactionCategory.INCOME))
        self.balance.apply_transaction(Transaction(40, TransactionCategory.INCOME))
